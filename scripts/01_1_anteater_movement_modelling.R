# This script processes the movement data for the giant anteaters
...
DATA <- read.csv("data/gps_data/anteater_gps_data.csv")
...
mp_IDs <- mp_data[which(mp_data$species == "Myrmecophaga_tridactyla"),"name"]
mp_IDs[mp_IDs %in% names(DATA)]
mp_IDs[!mp_IDs %in% names(DATA)]

#20 animals have gps data, 1 is missing gps data (this is correct)


#Define the file paths for saving the movement model outputs
Model_path <- paste("~/Dropbox/UBC/Projects/microplastics_brazil/results/anteater_movement/movement_models", sep = "")
HR_path <- paste("~/Dropbox/UBC/Projects/microplastics_brazil/results/anteater_movement/akdes", sep = "")
Fig_Path <- paste("~/Dropbox/UBC/Projects/microplastics_brazil/results/anteater_movement/figures", sep = "")
...
for(j in 1:length(DATA)){
  ...
  #Filter out any locations with very small time intervals
  DT <- c(Inf, diff(cilla$t))
  cilla <- cilla[DT > 5 %#% "min",]
  message("Dropped ", sum(DT < 5 %#% "min")," location(s) with short time intervals \n")
  
  #Plot the filtered location data for the current individual
  plot(cilla)
  ...
  RESULTS <- tryCatch(
    {
      RESULTS <- CTMM_FIT(cilla,
                          ...
                          HR_path = HR_path,
                          error = TRUE,
                          binomial = "Myrmecophaga_tridactyla")
    }, error=function(err) {
      message(cat("Model fitting failed, returning NAs \n"))
      
      RESULTS <- as.data.frame(t(unlist(c("Myrmecophaga_tridactyla",
                                          cilla@info[1],
                                          rep(NA, 23)))))
  ...
  if(j == 1){
    ...
    write.table(RESULTS,
                file = "~/Dropbox/UBC/Projects/microplastics_brazil/data/movement_summaries/anteater_movement_summary.csv",
                row.names=FALSE,
                col.names=TRUE,
                sep=",")
...
for(i in 1:length(DATA)){
  ...
  PATH <- file.path("~/Dropbox/UBC/Projects/microplastics_brazil/results/anteater_movement/akdes",
                    paste("AKDE_",
                          DATA[[i]]@info[1],
                          ".rda",
                          sep = ""))
  ...
  #---------
  #Expand the extent of the HR PDF by 20x
  e <- ext(HR3)
  cx <- (e$xmin + e$xmax) / 2
  cy <- (e$ymin + e$ymax) / 2
  hw_x <- (e$xmax - e$xmin) / 2 * 20
  hw_y <- (e$ymax - e$ymin) / 2 * 20
...
write.table(res,
            file = "~/Dropbox/UBC/Projects/microplastics_brazil/data/movement_summaries/anteater_land_use_summary.csv",
            row.names=FALSE,
            col.names=TRUE,
            sep=",")
