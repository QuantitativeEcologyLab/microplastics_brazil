...
for(i in 1:length(gps_data_paths)){
  ...
  for(j in 1:length(DATA)){
    ...
    land_cropped <- ifel(land_cropped %in% c(1,3,4,5,6,49), 99, land_cropped) #99 = Native_forest
    land_cropped <- ifel(land_cropped %in% c(12, 15), 98, land_cropped) #98 = Pasture
    land_cropped <- ifel(land_cropped %in% c(14, 18,19,20,21,39,40,41,62,36,46,47,35,48), 97, land_cropped) # 97 = Agriculture #9 = planted forests
    land_cropped <- ifel(land_cropped %in% c(24,25,30, 75), 96, land_cropped) # 96 = Development
    land_cropped <- ifel(land_cropped %in% c(11,26,33), 95, land_cropped) # 95 = water/wetland
    ...
    hfi_fig_path <- paste("~/Dropbox/UBC/Projects/microplastics_brazil/figures/tracking_data/hfi/",
                          species,
                          "/",
                          cilla@info$identity,
                          ".png",
    ...
    land_use_fig_path <- paste("~/Dropbox/UBC/Projects/microplastics_brazil/figures/tracking_data/land_use/",
                               species,
                               "/",
                               cilla@info$identity,
                               ".png",
