...
#library(RStoolbox) # For handling the habitat rasters
...
for(j in 1:length(DATA)){
  ...
  if(j == 1){
    ...
    write.table(RESULTS,
                file = "~/Dropbox/UBC/Projects/microplastics_brazil/data/movement_data/tapir_movement.csv",
                row.names=FALSE,
                col.names=TRUE,
                sep=",",
                append=TRUE)
...
write.table(res,
            file = "~/Dropbox/UBC/Projects/microplastics_brazil/data/movement_data/tapir_land_use.csv",
            row.names=FALSE,
            col.names=TRUE,
            sep=",")
